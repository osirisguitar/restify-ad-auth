...
/**
 * Returns an error if there's a problem, otherwise null.
 */
function validateOptions (options) {
  if (options.authenticationGroupDns) {
    if (!Array.isArray(options.authenticationGroupDns)) {
      return new Error('authenticationGroupDns is not an array. authenticationGroupDns is optional, but if specified it has to be an array of distinguishedNames (strings).');
    }

    if (!(options.serviceAccountDn && options.serviceAccountPassword)) {
      return new Error('Both serviceAccountDn and serviceAccountPassword are required.');
    }

    if (!options.ldapServerUrl) {
      return new Error('ldapServerUrl is required.')
    }

    if (!options.ldapBaseDn) {
      return new Error('ldapBaseDn is required.')
    }
  }
}

module.exports = {
  authenticate: function (userSAMAccountName, userPassword, options) {
